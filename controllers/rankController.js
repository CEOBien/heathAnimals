...
const rankController = {
  ...
  put: async (req, res) => {
    ...
    try {
      ...
      const updateRank = await Rank.findByIdAndUpdate(
        id,
        {
          level,
          name,
  ...
  search: async (req, res) => {
    try {
        const result = await Rank.find({ name: { $regex: req.params.key } });

        res.status(200).json({ mess: "Look for successfully", data: result });
    } catch (err) {
        console.log(err);
    }
