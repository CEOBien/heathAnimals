...
const bookingController = {
    add: async (req,res) => {
        ...
        const {startDate,endDate,day} = req.body;
        ...
        if (existingBookings.length > 0) {
          return res.json({mess: "You cannot book at this time on this day", status: false});
        }
        //tinh gio
        ...
        try {
            ...
            await Booking.updateOne({_id:saveBooking._id},{intermediaryToken:intermediarysSave._id});
            
            
            // Set a timeout to refund the user's money when the intermediary token expires
            setTimeout(async () => {
              // Find the intermediary token created earlier
              const expiredToken = await intermediarySchema.findOne({_id: intermediarysSave._id});
    ...
    handleBookingAccept: async (req, res) => {
        ...
        try {
          ...
          if (!booking || booking.status !== 'INACCEPT') {
            return res.status(404).json({ message: 'Booking not found or not in INACCEPT state' });
          }
          if(status === 'ACCEPT'){
            ...
            const findIdIntermediary = await intermediarySchema.findOne({_id:idIntermediary});
      ...
      getIdBooking: async (req,res) => {
        const id =req.params.id;
        try {
          const listBookingParter = await Booking.find({ny_id:id},{status:['INACCEPT', 'ACCPET']});
